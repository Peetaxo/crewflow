...
type CrewRatingsSnapshot = AppDataSnapshot & {
  crewRatings: TestCrewRating[];
};
...
describe('crew-ratings.service', () => {
  ...
  it('upserts a local event rating and recalculates the contractor average', async () => {
    let snapshot: CrewRatingsSnapshot = {
      events: [
        {
          id: 1,
    ...
    expect(snapshot.crewRatings.filter((rating: TestCrewRating) => rating.source === 'event')).toHaveLength(1);
