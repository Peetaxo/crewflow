    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      {mine.length === 0 ? (
        <div className="bg-white border border-gray-100 rounded-xl p-12 text-center shadow-sm">
          ...
          <p className="text-xs text-gray-500 mt-1">Zadne cekajici vykazy k vyrizeni.</p>
        </div>
      ) : isCrewHead ? (
              ...
              <div key={t.id} className="bg-white border border-gray-100 rounded-xl p-5 shadow-sm">
                <div className="flex items-center gap-3 mb-4 pb-4 border-b border-gray-50">
                  ...
                  <div className="ml-auto text-right">
                    ...
                    {t.km > 0 && <div className="text-[10px] text-gray-500">+ {formatCurrency(t.km * KM_RATE)} cestovne</div>}
                  </div>
                </div>
                ...
                <div className="flex gap-2">
                  <button onClick={() => handleTimelogAction(t.id, 'ch')} className="px-4 py-1.5 bg-emerald-600 text-white rounded-md text-xs font-medium hover:bg-emerald-700">Schvalit a poslat COO →</button>
                  <button onClick={() => handleTimelogAction(t.id, 'rej')} className="px-4 py-1.5 border border-red-100 text-red-600 rounded-md text-xs font-medium hover:bg-red-50">Zamitnout</button>
            ...
            return (
              <div key={g.event.id} className="bg-white border border-gray-100 rounded-xl p-5 shadow-sm">
                <div className="flex justify-between items-start mb-4 pb-3 border-b border-gray-100">
                  <div>
                    <div className="flex items-center gap-2"><span className="jn text-sm px-2 py-1">{g.event.job}</span><span className="text-base font-semibold">{g.event.name}</span></div>
                    <div className="text-xs text-gray-500 mt-1.5">{formatShortDate(g.event.startDate)} · {g.event.city} · {g.tls.length} kontraktoru</div>
                  </div>
                ...
                <div className="flex gap-2 mt-4">
                  <button onClick={() => approveAllTimelogs(g.event.id)} className="px-4 py-2 bg-emerald-600 text-white rounded-md text-xs font-medium hover:bg-emerald-700">Schvalit vse - {g.event.job} ({formatCurrency(totAmt)})</button>
                </div>
              </div>
            );
          })}
