...
const MobileCrewNav: React.FC<MobileCrewNavProps> = ({ badgeCounts, role = 'crew' }) => {
  ...
  const navItems = (isCrewNav ? crewMobileNavItemIds : managementNavItemIds)
  ...
  return (
    <nav className={navClassName} aria-label={isCrewNav ? 'Mobilní navigace Crew' : 'Mobilní navigace Management'}>
      {navItems.map((item) => {
