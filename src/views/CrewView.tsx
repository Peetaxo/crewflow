...
const CrewView = () => {
  const { selectedContractorId, setSelectedContractorId, filteredContractors, timelogs, setDeleteConfirm } = useAppContext();
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-gray-50">
            {filteredContractors.map(c => {
              const cTls = timelogs.filter(t => t.cid === c.id);
              return (
                <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => setSelectedContractorId(c.id)} className="px-2 py-1 border border-gray-200 rounded-lg text-[11px] font-medium hover:bg-gray-50 transition-colors">Detail</button>
                      <button onClick={() => setDeleteConfirm({ type: 'crew', id: c.id, name: c.name })} className="p-1.5 text-gray-300 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all" title="Smazat"><Trash2 size={14} /></button>
                    </div>
                  </td>
                </tr>
              );
