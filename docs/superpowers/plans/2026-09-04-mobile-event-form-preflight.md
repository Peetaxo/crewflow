...
- Návaznost přehledu směn je opravená v `f46184b` a `2a85e3d`: v2 bere nejbližší zbývající platný přiřazený den, časy řadí číselně, prázdný čas označuje „Čas zatím neurčen“, podporuje přesný UUID odkaz a při absenci budoucího přiřazení žádnou směnu nevymýšlí. Striktní kontrola data odmítá i nekanonické řetězce dříve přijímané `isMatch`; legacy větev se nemění.
- Opakované spec review schválilo opravy telefonu i přehledu (64 cílených testů). Hlavní agent poté ověřil celý suite na `2a85e3d`: 116 souborů / 1306 testů a build, oboje exit 0. Následuje poslední potvrzení quality review a integrace/instalace.
- Závěrečné quality review na `2a85e3d` schválilo sloučení bez dalších otevřených nálezů. Instalace zařízení zůstává posledním krokem; tento záznam ještě nepotvrzuje její výsledek.

## Dokončená integrace a vývojové instalace 2026-09-23

- Ověřená větev do `5ae1a29` byla fast-forward sloučena do `main` a odeslána do `origin/main`. Čistá instalační kopie `/Users/peetax/Projekty/crewflow-device-refresh-4S3gC4` ověřila sloučený main znovu: **116 souborů / 1306 testů, exit 0; build, exit 0**. Záznamy: `/private/tmp/crewflow-merged-main-tests-20260923.log` a `/private/tmp/crewflow-merged-main-build-20260923.log`.
- Nesouvisející rozpracované změny původní pracovní kopie v `TimelogsView.tsx`, jeho testu a `Info.plist` byly po integraci obnoveny. Kontrola přidaných/odebraných řádků proti stavu před integrací potvrdila shodu. Zachována souborová záloha v `/Users/peetax/Projekty/crewflow-local-backups/mobile-event-form/main-before-integration-20260923` i scoped stash `c7a8fe47979e554bd98393519e94acb0ee8d1121`; nejsou součástí commitů této dodávky.
- Z čistého a synchronizovaného main byl spuštěn povinný `npm run ios:refresh:devices` se zachovanou ignorovanou `.env.local` a stávajícím vývojovým týmem. První pokus se zasekl při webovém buildu; ukončen byl jen jeho vlastní proces Vite. Opakování stejného příkazu bez změny zdrojů úspěšně dokončilo web build, Capacitor sync a oba nativní buildy.
- **Simulátor iPhone 17 Pro: aktualizován a aplikace spuštěna.** Screenshot ověřil vykreslený přehled aplikace. Nativní formulář s otevřenou softwarovou klávesnicí nebyl samostatně proklikán: relace byla v roli Crew a změna rolí skutečného účtu není součástí autorizovaného testu. Vizuální a interakční testy formuláře proběhly v izolovaném mobilním webovém náhledu popsaném výše.
- **Fyzický iPhone 13 mini: aktualizován a aplikace spuštěna.** Build/podpis/instalace prošly, první spuštění odmítl iOS kvůli ověření vývojáře. Read-only kontrola mimo sandbox potvrdila platný podpis, odpovídající entitlements a profil s platností do 2026-09-30; chyby při kontrole uvnitř sandboxu nebyly vadou balíčku. Uživatel následně sám potvrdil důvěru na telefonu. Opakované `devicectl device process launch` pro `cz.nodu.app` skončilo exit 0 a výslovně potvrdilo spuštění; původní blokace instalace je tím vyřešena.
- Jde o lokální vývojové instalace, nikoli vydání do App Store. Databázové migrace jsou nasazené podle předchozího záznamu. Skutečné navazující COO účty se nezakládaly a role se neměnily; bez kvalifikovaného připojeného schvalovatele aplikace předání správně zablokuje. Globální typecheck má nadále 116 existujících diagnostik, nejde o zelený typecheck celého projektu.
